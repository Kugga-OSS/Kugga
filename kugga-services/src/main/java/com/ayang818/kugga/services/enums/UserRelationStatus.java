 ...
 * 3. other拒绝时将自己作为owner那条记录的status设置为fail，此时两条记录的status一条未success，一条未fail，双方好友关系建立失败
 * 4. 好友关系建立失败后，并不删除relation表中的数据，因为请求方需要知道自己请求失败。下一次添加好友的时候先检查，有没有存在申请记录，
 * 有的话则更新，没有的话则插入。
 * 5. owner发出好友申请后，处理完业务逻辑后尝试通过redis发布这条消息，若对方在线，直接推送过去这个请求，网页显示
 * 若不在线，则在对方下一次上线时主动拉去未处理的消息
